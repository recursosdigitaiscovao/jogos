...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if(container) {
        container.innerHTML = `
            <div style="position:relative; width:260px; height:140px; background:white; border-radius:20px; box-shadow:0 10px 25px rgba(0,0,0,0.05); display:flex; align-items:center; justify-content:center; border:4px solid #f0f7ff; overflow:hidden;">
...
function renderRound() {
    ...
    container.innerHTML = `
        <div style="display:flex; flex-direction:column; align-items:center; width:100%; gap:15px;">
            ...
            <!-- Armário/Estante -->
            <div id="shelf-container" style="position:relative; width:100%; max-width:650px; aspect-ratio: 500/165; background: url('${JOGO_CONFIG.caminhoImg}armarioletras2.png') no-repeat center; background-size: contain;">
                <div id="drop-zones" style="position:absolute; bottom:12%; left:4.5%; right:4.5%; height:75%; display:grid; grid-template-columns: repeat(4, 1fr); gap:12px; padding: 0 5px;">
                    ${[0,1,2,3].map(i => `
                        <div class="target-box" data-idx="${i}" onclick="removeItem(${i})"
                             style="height:100%; display:flex; align-items:center; justify-content:center; transition:0.2s; cursor:pointer;">
                        </div>
                    `).join('')}
                </div>
            </div>

            <!-- Cartões para ordenar -->
            <div id="drag-options" style="display:flex; gap:12px; flex-wrap:wrap; justify-content:center; padding:15px; background:rgba(255,255,255,0.4); border-radius:25px; width:100%; min-height:80px;">
                ${shuffled.map((item, i) => `
                    <div class="sort-card" 
                         onmousedown="startDrag(event)" ontouchstart="startDrag(event)"
                         onclick="handleItemClick(this)"
                         data-val="${item}" id="card-${i}"
                         style="background:white; padding:12px 15px; border-radius:12px; font-weight:900; font-size:18px; color:var(--primary-dark); cursor:grab; box-shadow:0 5px 0 #cbd9e6; border:2px solid #eee; min-width:65px; text-align:center; user-select:none; touch-action:none;">
                        ${item}
                    </div>
                `).join('')}
            </div>
...
function fillTarget(targetIdx, val, originalEl) {
    ...
    target.innerHTML = `
        <div style="background:var(--primary-blue); color:white; padding:10px 5px; border-radius:10px; font-weight:900; font-size:13px; width:90%; text-align:center; box-shadow:0 4px 0 var(--primary-dark); animation: popIn 0.3s; word-break: break-all;">
            ${val}
        </div>
    `;
