...
function renderizarEcraFabrica() {
    ...
    container.innerHTML = `
        <style>
            ...
            .station { display: flex; gap: 12px; justify-content: center; align-items: center; background: #f8fcff; border: 3px dashed var(--primary-blue); padding: 20px; border-radius: 25px; width: 100%; max-width: 480px; }
            .mold { width: 75px; height: 75px; background: white; border: 3px dashed #cbd5e1; border-radius: 18px; display: flex; align-items: center; justify-content: center; font-size: 1.8rem; font-weight: 950; color: var(--primary-blue); transition: 0.3s; }
            ...
            .bank { display: flex; flex-wrap: wrap; justify-content: center; gap: 12px; padding: 10px 0; width: 100%; flex-grow: 1; align-content: center; }
            .pill { background: white; border: 3px solid var(--primary-blue); color: var(--text-grey); border-radius: 22px; padding: 15px; min-width: 90px; font-size: 1.5rem; font-weight: 900; cursor: pointer; box-shadow: 0 5px 0 var(--primary-blue); }
            .pill:active { transform: translateY(3px); box-shadow: 0 2px 0 var(--primary-blue); }
            .warehouse { width: 100%; max-width: 600px; background: #f8fcff; border-radius: 22px; padding: 12px; border: 1px solid #eef2f6; min-height: 70px; }
            .tag { color: var(--text-grey); font-weight: 900; font-size: 0.9rem; text-transform: uppercase; border-bottom: 3px solid var(--primary-blue); padding: 2px 6px; margin: 4px; }
    ...
    resScreen.innerHTML = `
        <div class="res-inner">
            ...
            <div class="res-grid">
                ...
                <div class="res-card">
                    <span class="num" style="color: var(--cor-erro);">${erros}</span>
                    <span class="label">Erros</span>
                    <i class="fas fa-times-circle" style="color: var(--cor-erro);"></i>
                </div>
                <div class="res-card">
            ...
            <div class="res-actions">
                <button class="btn-res btn-res-play" onclick="location.reload()">
                    <i class="fas fa-sync-alt"></i> Jogar de Novo
                </button>
                <button class="btn-res btn-res-levels" onclick="openRDMenu()">
                    <i class="fas fa-chart-line"></i> Outro Nível
                </button>
                <a href="${JOGO_CONFIG.linkVoltar}" class="btn-res btn-res-exit">
