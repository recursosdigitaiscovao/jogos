...
window.atualizarAnimacao = function(chave) {
    ...
    `;
    const hand = document.getElementById('anim-hand'); const targetBtn = document.getElementById(`anim-btn-${cat.total}`);
    function rodarCiclo() {
...
function montarQuestao() {
    const area = document.getElementById('game-main-content'); const item = itensAtuais[indiceQuestao];
...
window.validarResposta = function(btn, num) {
    ...
    if (num === correta) {
        ...
        btn.style.background = "var(--highlight-green)"; new Audio(JOGO_CONFIG.sons.acerto).play().catch(e=>{});
    } else {
        erros++; document.getElementById('miss-val').innerText = erros;
        btn.style.background = "var(--error-red)"; new Audio(JOGO_CONFIG.sons.erro).play().catch(e=>{});
        botoes[correta - 1].style.border = "4px solid var(--highlight-green)";
    }
    setTimeout(() => {
        if (indiceQuestao < 9) { indiceQuestao++; montarQuestao(); } 
        else {
            clearInterval(cronometro); new Audio(JOGO_CONFIG.sons.vitoria).play().catch(e=>{});
