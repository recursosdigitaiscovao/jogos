...
function init() {
    ...
    // 1. Injeção de Textos no Header e Rodapé
    ...
    // 2. Configuração de Ícones e Links de Navegação
    ...
    // 3. GERAÇÃO DO MENU HAMBÚRGUER
    const menuBox = document.getElementById('dropdownMenu');
    menuBox.innerHTML = ''; 

    // --- Inserir Botão Voltar no Menu ---
    ...
    // 4. GERAÇÃO DO MENU DE TEMAS (RD MENU)
...
function genGrid() {
    ...
    cells.forEach(c => {
        ...
        d.onmousedown = (e) => { e.preventDefault(); startSel(d); }; 
        d.onmouseenter = () => contSel(d); 
        d.ontouchstart = (e) => { e.preventDefault(); startSel(d); }; 
        el.appendChild(d);
    });
...
function toggleHamburger(e) { 
    ...
    if(m.style.display === 'flex') document.getElementById('overlay').style.display = 'block';
}
...
function playSound(t) { 
    if(JOGO_CONFIG.sons[t]) new Audio(JOGO_CONFIG.sons[t]).play().catch(()=>{}); 
}
