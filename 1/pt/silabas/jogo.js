...
// Variáveis de Controlo do Arrastar
let draggingEl = null;
let startX = 0, startY = 0;
let moveThreshold = 10; // Pixels para considerar arrasto e não clique
let isMoving = false;
    ...
    area.innerHTML = `
        <div style="display:flex; flex-direction:column; align-items:center; width:100%; gap:25px; animation: fadeIn 0.4s;">
            ...
            <div id="slots-row" style="display:flex; gap:10px; justify-content:center; width:100%;">
                ${item.silabas.map((_, i) => `<div class="slot" onpointerup="handleSlotDrop(event)"></div>`).join('')}
            </div>
...
// LÓGICA HÍBRIDA (CLIQUE OU ARRASTO)
...
function moveInteraction(e) {
    ...
    if (isMoving) {
        draggingEl.style.transform = `translate(${curX - startX}px, ${curY - startY}px)`;
    }
}
...
function stopInteraction(e) {
    ...
    if (!isMoving) {
        ...
        // FOI UM ARRASTO
        const curX = e.clientX || e.changedTouches[0].clientX;
        const curY = e.clientY || e.changedTouches[0].clientY;
        const target = document.elementFromPoint(curX, curY);
        const slot = target ? target.closest('.slot') : null;

        if (slot && slot.innerText === "") {
            placeInSlot(draggingEl, slot);
        } else {
            draggingEl.style.transition = "0.3s";
            draggingEl.style.transform = "translate(0,0)";
        }
    }
...
function validar(tentativa) {
    ...
    if (tentativa.join('') === correta.join('')) {
        ...
        tocarSom('erro');
        setTimeout(() => {
            slots.forEach((s, i) => { s.innerText = correta[i]; s.style.background = "var(--highlight-green)"; });
        }, 500);
    }
    setTimeout(() => {
