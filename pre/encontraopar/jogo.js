...
function criarAnimacaoTutorial() {
    ...
    container.innerHTML = `
        ...
        <style>@keyframes tapP { 0%, 100% { transform: translate(0,0); } 50% { transform: translate(-10px,-15px) scale(1.1); } }</style>
    `;
}
...
function mostrarPergunta() {
    ...
    container.innerHTML = `
        <style>
            ...
            .options-grid { flex: 1; width: 100%; max-width: 900px; display: grid; grid-template-columns: repeat(4, 1fr); grid-template-rows: repeat(2, 1fr); gap: 10px; min-height: 0; padding: 5px; }
            .card-item { background: white; border: 2.5px solid #eee; border-radius: 18px; display: flex; align-items: center; justify-content: center; cursor: pointer; transition: 0.2s; box-shadow: 0 4px 0 #ddd; padding: 5px; box-sizing: border-box; height: 100%; width: 100%; min-height: 0; }
