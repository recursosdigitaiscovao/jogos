...
window.startLogic = function() {
    ...
    // Configurar ícone da Lâmpada na Barra de Status
    const timerBadge = document.querySelector('.badge-timer');
    if (timerBadge) {
        timerBadge.innerHTML = `<img src="${JOGO_CONFIG.caminhoImg}lampada.png" style="height:18px; cursor:pointer;" onclick="usarAjuda()"> <span id="ajuda-val">0</span>`;
    ...
    container.innerHTML = `
        <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; width:100%; height:100%; gap:15px;">
            <div style="height:100px; width:100px; background:white; border:3px solid var(--primary-blue); border-radius:20px; display:flex; align-items:center; justify-content:center;">
                <img src="${JOGO_CONFIG.caminhoImg}${itemTut.img}" style="height:70%; object-fit:contain;">
            </div>
...
window.initGame = function() {
    ...
    document.getElementById('ajuda-val').innerText = "0";
    mostrarPergunta();
};
...
function mostrarPergunta() {
    ...
    container.innerHTML = `
        <style>
            ...
            .target-zone { height: 25%; width: 100%; display: flex; align-items: center; justify-content: center; margin-bottom: 5px; }
            ...
            .options-grid { flex: 1; width: 100%; max-width: 850px; display: grid; grid-template-columns: repeat(4, 1fr); grid-template-rows: repeat(2, 1fr); gap: 10px; padding: 5px; }
            .card-item { background: white; border: 2.5px solid #eee; border-radius: 18px; display: flex; align-items: center; justify-content: center; cursor: pointer; transition: 0.2s; box-shadow: 0 4px 0 #ddd; padding: 8px; }
            ...
            @keyframes brilharAjuda { 0%, 100% { border-color: #eee; } 50% { border-color: #ff9f43; background: #fff5e6; transform: scale(1.05); } }
            @media (max-width: 500px) { .options-grid { grid-template-columns: repeat(2, 1fr); grid-template-rows: repeat(4, 1fr); } }
        </style>
...
function usarAjuda() {
    ...
    cards.forEach(card => {
        if (card.getAttribute('data-img') === alvo.img) {
            ...
            ajudasUtilizadas++;
            document.getElementById('ajuda-val').innerText = ajudasUtilizadas;
...
function finalizarJogo() {
    ...
    resScreen.innerHTML = `
        <style>
            ...
            .res-stats-grid { display: flex; gap: 10px; width: 100%; max-width: 360px; margin: 20px 0; }
            .stat-card { background: white; border-radius: 20px; padding: 15px 10px; flex: 1; text-align: center; box-shadow: 0 10px 20px rgba(0,0,0,0.05); border: 2px solid #f0f4f8; }
            ...
            .res-buttons { display: flex; flex-direction: column; gap: 12px; width: 100%; max-width: 300px; }
            .btn-res { padding: 16px; border-radius: 20px; font-weight: 900; font-size: 15px; cursor: pointer; border: none; text-align: center; text-decoration: none; text-transform: uppercase; transition: 0.2s; }
            .btn-blue { background: var(--primary-blue); color: white; box-shadow: 0 6px 0 var(--primary-dark); }
            .btn-outline { background: white; color: var(--primary-blue); border: 3px solid var(--primary-blue); box-shadow: 0 6px 0 var(--primary-blue); }
            .btn-grey { background: #e2e8f0; color: #64748b; box-shadow: 0 6px 0 #cbd5e1; }
            .btn-res:active { transform: translateY(3px); box-shadow: none; }
        </style>
        <div class="res-inner">
            <img src="${JOGO_CONFIG.caminhoImg}${rel.img}" style="height:120px; width:auto; margin-bottom:15px;">
            <h2 style="color: var(--primary-blue); font-weight:900; font-size:2rem; margin-bottom:5px;">${rel.titulo}</h2>
