...
// CSS DO MONSTRO (Corpo Único, Braços atrás, Reativo a Variáveis)
const CSS_MONSTRO = `
    .monster-container { 
        position: relative; width: 100%; height: 100%; 
        display: flex; flex-direction: column; align-items: center; justify-content: center;
        --monster-c: #66d1ed; /* Cor Base Azul */
    }
    /* Estilos Reativos */
...
function renderTutorialAnimation() {
    ...
    container.innerHTML = `
        <style>
            ...
            .tut-m-box { animation: monsterWait 5s infinite; }
            @keyframes monsterWait { 0%, 40% { transform: scale(0.8); } 45%, 65% { transform: scale(0.85); } 70%, 100% { transform: scale(0.8); } }
        </style>
        <div class="tut-stage">
    ...
    `;
    setInterval(() => {
        const t = document.getElementById('tut-monster-parent');
...
function renderizarEcraAlimentacao() {
    ...
    container.innerHTML = `
        <style>
            ...
            .m-size { width: clamp(210px, 35vh, 290px); height: clamp(230px, 38vh, 320px); }
            .food-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 15px; }
            .food-card { width: clamp(100px, 19vh, 150px); height: clamp(100px, 19vh, 150px); background: white; border: 3px solid #f0f4f8; border-radius: 28px; display: flex; align-items: center; justify-content: center; box-shadow: 0 6px 0 #d0d8de; position: relative; }
            .food-img { width: 95%; height: 95%; object-fit: contain; cursor: pointer; touch-action: none; }
            ...
            .flying { position: fixed !important; z-index: 1000; width: 80px !important; height: 80px !important; pointer-events: none; transition: all 0.5s ease-in; transform: translate(-50%, -50%) scale(0.4); }
            @media (max-width: 650px) { .food-grid { grid-template-columns: repeat(3, 1fr); gap: 8px; } .m-size { width: 150px; height: 170px; } .food-card { width: 85px; height: 85px; } }
...
function processarAlimentacao(el, imediato = false) {
    ...
    if (status === 'correto') {
        ...
        parent.classList.add('happy'); // FICA VERDE
        el.remove();
        alvosRestantesNaRonda--;
    } else {
        erros++; somErro.play();
        parent.classList.add('sad'); // FICA VERMELHO
    ...
    res.innerHTML = `
        <div class="screen-box" style="justify-content: center; padding: 20px; display: flex !important;">
            <div style="display:flex; flex-direction:column; align-items:center; width:100%; max-width:450px; margin:auto;">
                <img src="${JOGO_CONFIG.caminhoImg}${rank.img}" style="width:130px; display: block; margin: 0 auto;">
                ...
                <div style="display:flex; gap:12px; margin-bottom:30px; width:100%; justify-content:center;">
                    ...
                    <div style="background:white; border-radius:20px; width:105px; height:105px; display:flex; flex-direction:column; align-items:center; justify-content:center; box-shadow: 0 8px 20px rgba(0,0,0,0.05);"><span style="font-size:1.6rem; font-weight:900; color:#ff5e5e;">${erros}</span><span style="font-size:0.6rem; font-weight:900; color:#88a; text-transform:uppercase;">Errados</span></div>
                    <div style="background:white; border-radius:20px; width:105px; height:105px; display:flex; flex-direction:column; align-items:center; justify-content:center; box-shadow: 0 8px 20px rgba(0,0,0,0.05);"><span style="font-size:1.6rem; font-weight:900; color:#ff9f43;">${ajudasUtilizadas}</span><span style="font-size:0.6rem; font-weight:900; color:#88a; text-transform:uppercase;">Ajudas</span></div>
                </div>
