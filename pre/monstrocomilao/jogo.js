...
function falarInstrucao() {
    if ('speechSynthesis' in window) {
        ...
        const msg = new SpeechSynthesisUtterance(JOGO_CATEGORIAS[categoriaAtiva].description || JOGO_CATEGORIAS[categoriaAtiva].descricao);
...
function renderTutorialAnimation() {
    ...
    container.innerHTML = `
        <style>
            ...
            .tut-label { font-weight: 900; color: var(--primary-blue); text-transform: uppercase; font-size: 1rem; }
            .tut-stage { position: relative; width: 320px; height: 160px; background: white; border: 3px dashed var(--primary-blue); border-radius: 25px; display: flex; align-items: center; justify-content: center; overflow: hidden; }
            .tut-m-box { width: 90px; height: 110px; margin-left: -100px; transform: scale(0.8); }
            .tut-f { position: absolute; width: 40px; z-index: 15; left: 220px; top: 60px; animation: fS 5s infinite; }
            .tut-h { position: absolute; font-size: 35px; z-index: 20; left: 230px; top: 90px; animation: hS 5s infinite; }
            @keyframes hS { 0%, 15% { left: 230px; top: 90px; } 45%, 65% { left: 55px; top: 75px; } 80%, 100% { left: 230px; top: 90px; } }
            @keyframes fS { 0%, 15% { left: 220px; top: 60px; opacity: 1; } 45% { left: 35px; top: 65px; opacity: 1; transform: scale(0.6); } 50%, 100% { left: 35px; top: 65px; opacity: 0; } }
        </style>
        <div class="tut-wrapper">
            <div class="tut-label">MONSTRINHO COMILÃO</div>
            <div class="tut-stage">
                <div class="tut-m-box">
...
function renderizarEcraAlimentacao() {
    ...
    container.innerHTML = `
        <style>
            ${CSS_MONSTRO}
            .monster-wrap { width: 100%; height: 100%; display: flex; flex-direction: column; align-items: center; justify-content: space-around; padding: 5px; box-sizing: border-box; }
            .instr-box { background: white; padding: 8px 20px; border-radius: 50px; font-weight: 900; color: #5d7082; border: 3px solid var(--primary-blue); display: flex; align-items: center; gap: 12px; box-shadow: 0 4px 10px rgba(0,0,0,0.05); }
            
            .btn-som { 
                background: #ff9800; border: none; width: 42px; height: 42px; 
                border-radius: 50% !important; color: white; cursor: pointer; 
                box-shadow: 0 4px 0 #e65100; display: flex; align-items: center; justify-content: center; flex-shrink: 0;
            }
            ...
            .m-size { width: clamp(180px, 32vh, 260px); height: clamp(200px, 35vh, 290px); }
            ...
            /* PORTRAIT (Mobile): Monstro em cima, Comida em baixo */
            @media (max-width: 650px) and (orientation: portrait) {
                .game-stage { flex-direction: column !important; gap: 10px; }
                .m-size { width: 140px; height: 160px; }
                .food-grid { grid-template-columns: repeat(3, 1fr); gap: 8px; }
                .food-card { width: 85px; height: 85px; border-radius: 18px; }
                .instr-box { width: 98%; padding: 6px 15px; }
            ...
            .flying { position: fixed !important; z-index: 1000; width: 80px !important; height: 80px !important; pointer-events: none; transition: all 0.6s cubic-bezier(0.2, 0, 0.2, 1); transform: translate(-50%, -50%) scale(0.5); }
        </style>
...
function voarAteBoca(el) {
    ...
    // Configura o elemento para voar
    el.style.width = "80px";
    ...
    // Pequeno delay para o browser registar a posição inicial antes da transição
    setTimeout(() => {
        el.style.left = (mR.left + mR.width/2) + "px";
        el.style.top = (mR.top + mR.height/2) + "px";
...
function processarAlimentacao(el, imediato = false) {
    ...
    setTimeout(() => {
        if (status === 'correto') {
            ...
            parent.classList.add('happy'); el.remove(); alvosRestantesNaRonda--;
        } else {
            erros++; somErro.play();
            parent.classList.add('sad'); 
            el.classList.remove('flying', 'dragged');
        ...
        setTimeout(() => {
            parent.classList.remove('happy', 'sad', 'open');
            jogoAtivo = true;
    ...
    resScreen.innerHTML = `
        <div class="screen-box" style="justify-content: center; padding: 20px; display: flex !important;">
            <div style="display:flex; flex-direction:column; align-items:center; width:100%; max-width:450px; margin:auto;">
                <img src="${JOGO_CONFIG.caminhoImg}${rank.img}" style="width:110px; display:block; margin: 0 auto 5px;">
                <h1 style="color:var(--primary-blue); font-weight:900; font-size:1.8rem; margin:10px 0; text-align:center;">${rank.titulo}</h1>
                <div style="display:flex; gap:10px; margin-bottom:25px; width:100%; justify-content:center;">
                    <div style="background:white; border-radius:20px; width:90px; height:90px; display:flex; flex-direction:column; align-items:center; justify-content:center; box-shadow: 0 8px 20px rgba(0,0,0,0.05); border:1px solid #f0f0f0;"><span style="font-size:1.6rem; font-weight:900; color:#7ed321;">${acertos}</span><span style="font-size:0.6rem; font-weight:900; color:#88a; text-transform:uppercase;">Certos</span></div>
                    <div style="background:white; border-radius:20px; width:90px; height:90px; display:flex; flex-direction:column; align-items:center; justify-content:center; box-shadow: 0 8px 20px rgba(0,0,0,0.05); border:1px solid #f0f0f0;"><span style="font-size:1.6rem; font-weight:900; color:#ff5e5e;">${erros}</span><span style="font-size:0.6rem; font-weight:900; color:#88a; text-transform:uppercase;">Errados</span></div>
                    <div style="background:white; border-radius:20px; width:90px; height:90px; display:flex; flex-direction:column; align-items:center; justify-content:center; box-shadow: 0 8px 20px rgba(0,0,0,0.05); border:1px solid #f0f0f0;"><span style="font-size:1.6rem; font-weight:900; color:#ff9f43;">${ajudasUtilizadas}</span><span style="font-size:0.6rem; font-weight:900; color:#88a; text-transform:uppercase;">Ajudas</span></div>
                </div>
                <button style="height:55px; border-radius:30px; font-size:1.1rem; width:100%; max-width:300px; background:var(--primary-blue); color:white; border:none; box-shadow:0 6px 0 var(--primary-dark); font-weight:900; cursor:pointer;" onclick="location.reload()"><i class="fas fa-redo"></i> JOGAR DE NOVO</button>
                <button style="height:55px; border-radius:30px; font-size:1.1rem; background:white; color:var(--primary-blue); border:3.5px solid var(--primary-blue); margin: 8px 0; width:100%; max-width:300px; font-weight:900; cursor:pointer;" onclick="openRDMenu()"><i class="fas fa-chart-line"></i> OUTRO NÍVEL</button>
                <a href="${JOGO_CONFIG.linkVoltar}" style="height:55px; border-radius:30px; font-size:1.1rem; background:#e2e8f0; color:#64748b; text-decoration:none; display:flex; align-items:center; justify-content:center; width:100%; max-width:300px; font-weight:900;"><i class="fas fa-sign-out-alt"></i> SAIR</a>
            </div>
        </div>
    `;
}
