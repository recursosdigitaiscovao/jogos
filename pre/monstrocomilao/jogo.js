...
function renderTutorialAnimation() {
    ...
    container.innerHTML = `
        <style>
            ...
            .tut-food { position: absolute; width: 50px; z-index: 15; left: 220px; top: 70px; transition: transform 0.2s; }
            ...
            @keyframes handSequence {
                ...
                10% { left: 240px; top: 85px; transform: scale(0.9); } /* Pega o alimento */
                40%, 60% { left: 60px; top: 95px; transform: scale(0.9); } /* Arrasta até à boca */
                70% { left: 240px; top: 100px; transform: scale(1); } /* Volta */
                100% { left: 240px; top: 100px; }
            }
            @keyframes foodSequence {
                0% { left: 220px; top: 70px; opacity: 1; transform: scale(1); }
                10% { left: 220px; top: 70px; opacity: 1; transform: scale(1); }
                40% { left: 40px; top: 80px; opacity: 1; transform: scale(0.8); }
                45%, 100% { left: 40px; top: 80px; opacity: 0; transform: scale(0.4); } /* Engolido */
            }
            @keyframes mouthSequence {
                0%, 35% { height: 15px; }
                40%, 60% { height: 40px; } /* Abre para comer */
                65%, 100% { height: 15px; }
            }
...
function renderizarEcraAlimentacao() {
    ...
    container.innerHTML = `
        <style>
            ...
            .monster { width: clamp(190px, 35vh, 280px); height: clamp(190px, 35vh, 280px); background: #9c27b0; border-radius: 50% 50% 40% 40%; position: relative; box-shadow: 0 14px 0 #4a148c, inset 0 -10px 20px rgba(0,0,0,0.2); display: flex; flex-direction: column; align-items: center; justify-content: center; transition: 0.3s; }
            ...
            @media (max-width: 600px) { .monster { width: 170px; height: 170px; } .food-card { width: 90px; height: 90px; } }
        </style>
...
function iniciarArrasto(e, el) {
    ...
    function mover(e) {
        ...
        if (Math.abs(cX - sX) > 10 || Math.abs(cY - sY) > 10) {
            moved = true; el.classList.add('dragged'); el.style.position = 'fixed'; el.style.zIndex = '1000';
            el.style.left = (cX - oX) + "px"; el.style.top = (cY - oY) + "px"; el.style.transform = "scale(1.1)";
    ...
    function soltar() { 
        ...
        if (moved) { el.style.transform = "scale(1)"; verificarColisao(el); } 
